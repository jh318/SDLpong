...
bool LoadFiles()
{
	...
	PlayerPaddleImage = LoadImage("graphics/PlayerPaddleSmile.bmp");
	EnemyPaddleImage = LoadImage("graphics/EnemyPaddleFrown.bmp");
	...
	//Load sounds
	BallBounceSound = Mix_LoadWAV("audio/explosion1.wav");
	BallSpawnSound = Mix_LoadWAV("audio/explosion2.wav");
...
bool InitSDL()
{
	...
	Window = SDL_CreateWindow("Paddles of Warth", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
...
void DrawGame()
{
	...
	sprintf_s(playerHUD, "Player Score: %d", PlayerScore);
	sprintf_s(enemyHUD, "Enemy Score: %d", EnemyScore);
	if (PlayerScore > EnemyScore) {
		DrawText(Backbuffer, playerHUD, 0, 1, GameFont, 0, 255, 0);
		DrawText(Backbuffer, enemyHUD, 0, 30, GameFont, 255, 0, 64);
	}
	else if (EnemyScore > PlayerScore) {
		DrawText(Backbuffer, playerHUD, 0, 1, GameFont, 255, 0, 0);
		DrawText(Backbuffer, enemyHUD, 0, 30, GameFont, 0, 255, 0);
	}
	else 
	{
		DrawText(Backbuffer, playerHUD, 0, 1, GameFont, 0, 0, 0);
		DrawText(Backbuffer, enemyHUD, 0, 30, GameFont, 0, 0, 0);
	}
...
void FreeGame()
{
	SDL_DestroyWindow(Window);	//SDL_DestroyWindow destroys a window we created with SDL_CreateWindow
